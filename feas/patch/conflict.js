...
let out = mergedFile(fs.readFileSync('test/b.txt',"utf-8"),fs.readFileSync('test/current.txt',"utf-8"),fs.readFileSync('test/incoming.txt',"utf-8"));
console.log(`conflict: ${out.conflict}`);
fs.writeFileSync('out.txt',out.newContent);
...
function findmaxnum(oldLines,i){
  ...
  let currminus = line.curr.minus != undefined;
  let ret = 0;
  console.log(( incomingminus|| incomingplus || currminus || currplus ) && (i in oldLines));
  while(( incomingminus|| incomingplus || currminus || currplus ) && (i in oldLines)){
    console.log(i);
    i++;
    line = oldLines[i];
